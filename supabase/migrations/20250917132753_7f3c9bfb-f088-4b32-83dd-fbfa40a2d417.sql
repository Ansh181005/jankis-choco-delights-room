-- Fix critical security issues (handling all existing data)

-- 1. Fix existing phone data that violates constraints
UPDATE public.contacts 
SET phone = '+91' || phone 
WHERE phone IS NOT NULL 
AND phone !~ '^[+]?[0-9\s\-\(\)]{10,20}$'
AND char_length(phone) >= 8;

-- Delete records with completely invalid phone numbers
...
-- 3. Add database constraints for security and data integrity (with more lenient phone format)
...
ALTER TABLE public.contacts 
ADD CONSTRAINT check_phone_format CHECK (phone IS NULL OR char_length(phone) >= 10);
